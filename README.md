...
может вернуть `429`. При `401/403/429` клиент делает не более трёх повторов в той
же сессии с backoff, jitter и поддержкой `Retry-After`, сохраняя выданные QRATOR
cookies. Весь цикл ограничен общим deadline. Cookies существуют только в памяти
процесса и на диск не записываются.
